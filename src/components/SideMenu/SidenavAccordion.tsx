...
export const SidenavAccordion = ({ item, isExpanded, setIsExpanded, defaultExpanded }: Props) => {
  ...
  useEffect(() => {
    if (item) {
      void getConfig('routes', item).then((res) =>
        setFilteredRoutes(config.filter(({ key }) => res?.includes(key)))
      );
    }
  }, [item]);
          ...
          {filteredRoutes &&
            filteredRoutes.map(({ key, title, route, icon, dataTestId }) => (
              <SidenavItem
                ...
                isSelected={history.location.pathname.startsWith(
                  `${BASE_ROUTE}/${initiativeId}/${route}`
                )}
                icon={icon}
                level={2}
